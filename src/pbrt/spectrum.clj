(ns pbrt.spectrum
  ...
  (:import [clojure2d.math.vector Vec2 Vec3 ArrayVec]))

(set! *warn-on-reflection* true)
(set! *unchecked-math* :warn-on-boxed)
(m/use-primitive-operators)

(defprotocol SegmentProto
  (get-val [s v] [s v line?] "Get value from segment. If value is outside the range, return constant value (default) or treat as linear function defined by segment.")
  (intersect [s1 start end] "Intersect two intervals on x axis, returns nil if there is no intersection.")
  (inside? [s1 v] "Check if value is within range."))

(declare make-segment)

(defrecord LineSegment [^double x1 ^double x2 ^double val1 ^double val2 ^double dx]
  SegmentProto
  (get-val [_ v line?]
    (let [nv (m/lerp val1 val2 (/ (- ^double v x1) dx))]
      (if line?
        nv
        (cond
          (< ^double v x1) val1
          (> ^double v x2) val2
          :else nv))))
  (get-val [s v] (get-val s v false))
  (intersect [s start end]
    (let [i1 (max x1 ^double start)
          i2 (min x2 ^double end)]
      (if (>= i1 i2) 
        nil
        (make-segment i1 i2 (get-val s i1) (get-val s i2)))))
  (inside? [_ v] (bool-and (>= ^double v x1)
                           (<= ^double v x2))))

(defn make-segment
  "Create segment"
  [^double x1 ^double x2 val1 val2]
  (->LineSegment x1 x2 val1 val2 (- x2 x1)))

(defn prepare-samples
  "Prepare segments structure from samples"
  [lambda vals]
  (let [segments (map #(let [[x1 x2] %1
                             [v1 v2] %2]
                         (make-segment x1 x2 v1 v2))
                      (map vector lambda (next lambda))
                      (map vector vals (next vals)))
        fs (first segments)
        [mns mxs] (reduce #(let [[mns mxs] %1
                                 nmns (if (< ^double (:x1 %2) ^double (:x1 mns)) %2 mns)
                                 nmxs (if (> ^double (:x2 %2) ^double (:x2 mxs)) %2 mxs)]
                             [nmns nmxs]) [fs fs] (next segments))]
    {:mn mns
     :mx mxs
     :segments segments}))

(def cie-lambda (range 360 831))

(def cie-x-raw (double-array [0.0001299000   0.0001458470   0.0001638021   0.0001840037
...
(def cie-x (prepare-samples cie-lambda cie-x-raw))

(def cie-y-raw (double-array [0.000003917000  0.000004393581  0.000004929604  0.000005532136
...
(def cie-y (prepare-samples cie-lambda cie-y-raw))

(def cie-z-raw (double-array [0.0006061000 0.0006808792 0.0007651456 0.0008600124
...
(def cie-z (prepare-samples cie-lambda cie-z-raw))

(def ^:const ^double cie-y-integral 106.856895)
(def ^:const ^int n-cie-samples (count cie-lambda))
(def ^:const ^double cie-min-lambda (reduce clojure.core/min cie-lambda))
(def ^:const ^double cie-max-lambda (reduce clojure.core/max cie-lambda))
(def ^:const ^double cie-scale (/ (- cie-max-lambda cie-min-lambda)
                                  (* cie-y-integral n-cie-samples)))


(def rgb-lambda [380.000000, 390.967743, 401.935486, 412.903229, 423.870972, 434.838715,
...
(def rgb-refl-to-spect-white (prepare-samples rgb-lambda [1.0618958571272863e+00, 1.0615019980348779e+00, 1.0614335379927147e+00,
...
(def rgb-refl-to-spect-cyan (prepare-samples rgb-lambda [1.0414628021426751e+00,  1.0328661533771188e+00,  1.0126146228964314e+00,
...
(def rgb-refl-to-spect-magenta (prepare-samples rgb-lambda [9.9422138151236850e-01,  9.8986937122975682e-01, 9.8293658286116958e-01,
...
(def rgb-refl-to-spect-yellow (prepare-samples rgb-lambda [5.5740622924920873e-03,  -4.7982831631446787e-03, -5.2536564298613798e-03,
...
(def rgb-refl-to-spect-red (prepare-samples rgb-lambda [1.6575604867086180e-01,  1.1846442802747797e-01,  1.2408293329637447e-01,
...
(def rgb-refl-to-spect-green (prepare-samples rgb-lambda [2.6494153587602255e-03,  -5.0175013429732242e-03, -1.2547236272489583e-02,
...
(def rgb-refl-to-spect-blue (prepare-samples rgb-lambda [9.9209771469720676e-01,  9.8876426059369127e-01,  9.9539040744505636e-01,
...
(def rgb-illum-to-spect-white (prepare-samples rgb-lambda [1.1565232050369776e+00, 1.1567225000119139e+00, 1.1566203150243823e+00,
...
(def rgb-illum-to-spect-cyan (prepare-samples rgb-lambda [1.1334479663682135e+00,  1.1266762330194116e+00,  1.1346827504710164e+00,
...
(def rgb-illum-to-spect-magenta (prepare-samples rgb-lambda [1.0371892935878366e+00,  1.0587542891035364e+00,  1.0767271213688903e+00,
...
(def rgb-illum-to-spect-yellow (prepare-samples rgb-lambda  [2.7756958965811972e-03,  3.9673820990646612e-03,  -1.4606936788606750e-04,
...
(def rgb-illum-to-spect-red (prepare-samples rgb-lambda  [5.4711187157291841e-02,  5.5609066498303397e-02,  6.0755873790918236e-02,
...
(def rgb-illum-to-spect-green (prepare-samples rgb-lambda  [2.5168388755514630e-02,  3.9427438169423720e-02,  6.2059571596425793e-03,
...
(def rgb-illum-to-spect-blue (prepare-samples rgb-lambda  [1.0570490759328752e+00,  1.0538466912851301e+00,  1.0550494258140670e+00,
...
(defn average-spectrum-samples
  "Return average value of fn made of linear segments"
  ([{:keys [mn mx segments]} ^double lambda-start ^double lambda-end line?]
   (let [segments (if (< lambda-start ^double (:x1 mn))
                    (conj segments (make-segment lambda-start (:x1 mn) (get-val mn lambda-start line?) (:val1 mn)))
                    segments)
         segments (if (> lambda-end ^double (:x2 mx))
                    (conj segments (make-segment (:x2 mx) lambda-end (:val2 mx) (get-val mx lambda-end line?)))
                    segments)
         segments (remove nil? (map #(intersect %1 lambda-start lambda-end) segments))]
     (/ ^double (reduce #(let [s %2]                       
                           (+ ^double %1 (* 0.5 ^double (:dx s) (+ ^double (:val1 s) ^double (:val2 s))))) 0.0 segments)
        (- lambda-end lambda-start))))
  ([samples lambda-start lambda-end] (average-spectrum-samples samples lambda-start lambda-end false)))

(defn interpolate-spectrum-samples
  "Return value of fn made of linear segments"
  ([{:keys [mn mx segments]} ^double l line?]
   (cond
     (<= l ^double (:x1 mn)) (get-val mn l line?)
     (>= l ^double (:x2 mx)) (get-val mx l line?)
     :else (get-val (first (filter #(inside? % l) segments)) l)))
  ([samples l] (interpolate-spectrum-samples samples l false)))

(defn resample-linear-spectrum
  "Resample segments using average-spectrum-samples. This is simplified version comparing to original PBRT function. Not used anywhere."
  ([samples lambda-start lambda-end no-of-samples line?]
   (let [^double lambda-start lambda-start
         ^double lambda-end lambda-end
         ^long no-of-samples no-of-samples
         step (/ (- lambda-end lambda-start) (dec no-of-samples))
         delta (/ step 2.0)
         lambdas (map #(m/lerp lambda-start lambda-end (/ (double %) (dec no-of-samples))) (range no-of-samples))]
     {:lambdas lambdas
      :values (map #(average-spectrum-samples samples (- ^double % delta) (+ ^double % delta) line?) lambdas)}))
  ([samples lambda-start lambda-end no-of-samples] (resample-linear-spectrum samples lambda-start lambda-end no-of-samples false)))

(defn simple-resample
  "Resample via average-spectrum-samples. Returns list of values."
  ([samples lambda-start lambda-end no-of-samples line?]
   (for [i (range no-of-samples)
         :let [wl0 (m/lerp lambda-start lambda-end (/ (double i) ^long no-of-samples))
               wl1 (m/lerp lambda-start lambda-end (/ (inc (double i)) ^long no-of-samples))]]
     (average-spectrum-samples samples wl0 wl1 line?)))
  ([samples lambda-start lambda-end no-of-samples] (simple-resample samples lambda-start lambda-end no-of-samples false)))
...
(extend Vec3
  ...
  {:y c/to-luma
   ...
   :from-rgb identity
   :to-xyz c/to-XYZ-raw
   :from-xyz c/from-XYZ-raw
   :black? v/is-zero?})

(defn rgb-from-sampled
  "Return color from sampled spd"
  [samples]
  (let [xyz (reduce #(let [idx (- (int %2) cie-min-lambda)
                           v (interpolate-spectrum-samples samples %2)
                           x (aget ^doubles cie-x-raw idx) 
                           y (aget ^doubles cie-y-raw idx) 
                           z (aget ^doubles cie-z-raw idx)]
                       (v/add %1 (v/mult (Vec3. x y z) v))) (Vec3. 0.0 0.0 0.0) cie-lambda)]
    (from-xyz (v/mult xyz cie-scale))))
